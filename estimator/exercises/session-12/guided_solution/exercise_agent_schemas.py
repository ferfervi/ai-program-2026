"""Schemas for the Session 12 agent — EXERCISE copy (build-it-yourself).

This is a self-contained re-creation of ``app/generation/agentic/agent_schemas.py``
that lives inside the exercise folder so you can rebuild the agent without touching
the reference solution. It is GIVEN in full: these are plain Pydantic models, not the
part of the exercise you are meant to write. The learning happens in
...
* **Result models** (``AgentComponent`` / ``AgentEstimate`` / ``AgentRunResult``)
  — a deliberately LIGHT final estimate. The terminal ``responses.parse`` call in
  the loop fills ``AgentEstimate``.
"""
...
# Strip NUL / control characters a model may occasionally emit inside a malformed
# unicode escape, so a model glitch never poisons the readable trace.
...
class AgentRunResult(BaseModel):
    ...
    # "scaffold_incomplete" is exercise-only: an intermediate milestone stopped the
    # loop on purpose before it could produce an estimate. Absent in the solution.
    stopped_reason: Literal[
        "completed", "max_iterations", "no_final_estimate", "scaffold_incomplete"
    ] = "completed"
