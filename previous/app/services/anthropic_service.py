...
class AnthropicEstimator:
    def __init__(self, client = Anthropic(api_key=get_settings().ANTHROPIC_API_KEY)):
    ...
    def estimate(self, messages: List[Dict[str, str]]) -> AnthropicEstimation:
        ...
        if getattr(response, "completion", None):
            return AnthropicEstimation(
                ...
                provider_info=LLMProviderInfo(provider="anthropic", model=self.settings.LLM_MODEL),
